...
impl LanguageRefExtractor for TypeScriptExtractor {
    ...
    fn resolve_import(&self, import_path: &str, workspace: &Path) -> Option<PathBuf> {
        ...
        if !clean.starts_with('.') { return None; }
    ...
    fn walk(node: &Node, source: &str, current: &Option<String>, map: &mut Vec<Option<String>>) {
        ...
        let name = match kind {
            "function_declaration" | "method_definition" | "arrow_function" => {
                node.child_by_field_name("name").map(|n| source[n.start_byte()..n.end_byte()].to_string())
            }
            _ => None,
        };
        ...
        if let Some(n) = active {
            for line in node.start_position().row..=node.end_position().row.min(map.len() - 1) {
                if map[line].is_none() { map[line] = Some(n.clone()); }
            }
        }
        let mut cursor = node.walk();
        for child in node.children(&mut cursor) {
            walk(&child, source, &name.clone().or_else(|| current.clone()), map);
        }
    }
    ...
    match kind {
                ...
                match func.kind() {
                    "identifier" => {
                        ...
                        if !is_builtin_js(&name) {
                            refs.push(RawReference { line, source_symbol: enclosing.clone(), target_name: name, target_qualifier: None, kind: RefKind::Call });
                        }
                    }
                    "member_expression" => {
                        if let (Some(obj), Some(prop)) = (func.child_by_field_name("object"), func.child_by_field_name("property")) {
                            refs.push(RawReference { line, source_symbol: enclosing.clone(), target_name: text(&prop, source), target_qualifier: Some(text(&obj, source)), kind: RefKind::Call });
                        }
                    }
                    _ => {}
                }
        ...
        "new_expression" => {
            if let Some(constructor) = node.child_by_field_name("constructor") {
                refs.push(RawReference { line, source_symbol: enclosing.clone(), target_name: text(&constructor, source), target_qualifier: None, kind: RefKind::Call });
            }
        }
        "type_identifier" => {
            let name = text(node, source);
            if !is_builtin_ts_type(&name) {
                refs.push(RawReference { line, source_symbol: enclosing.clone(), target_name: name, target_qualifier: None, kind: RefKind::TypeRef });
            }
        }
        "import_statement" => {
            if let Some(src) = node.child_by_field_name("source") {
                refs.push(RawReference { line, source_symbol: None, target_name: text(&src, source).trim_matches(|c| c == '\'' || c == '"').to_string(), target_qualifier: None, kind: RefKind::Import });
            }
        }
        _ => {}
    }
...
fn text(node: &Node, source: &str) -> String { source[node.start_byte()..node.end_byte()].to_string() }

fn is_builtin_js(name: &str) -> bool {
    matches!(name, "console" | "require" | "parseInt" | "parseFloat" | "setTimeout" | "setInterval"
        | "clearTimeout" | "clearInterval" | "Promise" | "Array" | "Object" | "JSON" | "Math"
        | "String" | "Number" | "Boolean" | "Symbol" | "Map" | "Set" | "Date" | "RegExp" | "Error"
        | "undefined" | "null" | "NaN" | "Infinity" | "isNaN" | "isFinite" | "encodeURIComponent" | "decodeURIComponent")
}

fn is_builtin_ts_type(name: &str) -> bool {
    matches!(name, "string" | "number" | "boolean" | "void" | "any" | "unknown" | "never"
