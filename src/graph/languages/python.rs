...
impl LanguageRefExtractor for PythonExtractor {
    ...
    fn resolve_import(&self, _import_path: &str, _workspace: &Path) -> Option<PathBuf> { None }
    ...
    match kind {
        "call" => {
            if let Some(func) = node.child_by_field_name("function") {
                match func.kind() {
                    ...
                    "attribute" => {
                        if let (Some(obj), Some(attr)) = (func.child_by_field_name("object"), func.child_by_field_name("attribute")) {
                            refs.push(RawReference { line, source_symbol: enclosing.clone(), target_name: text(&attr, source), target_qualifier: Some(text(&obj, source)), kind: RefKind::Call });
                        }
                    }
                    _ => {}
                }
        ...
        "import_from_statement" => {
            if let Some(module) = node.child_by_field_name("module_name") {
                refs.push(RawReference { line, source_symbol: None, target_name: text(&module, source), target_qualifier: None, kind: RefKind::Import });
            }
        }
        "import_statement" => {
            let mut cursor = node.walk();
            for child in node.children(&mut cursor) {
                if child.kind() == "dotted_name" {
                    refs.push(RawReference { line, source_symbol: None, target_name: text(&child, source), target_qualifier: None, kind: RefKind::Import });
                }
            }
        }
        _ => {}
...
fn is_builtin_py(name: &str) -> bool {
    matches!(name, "print" | "len" | "range" | "int" | "str" | "float" | "bool" | "list" | "dict"
        | "set" | "tuple" | "type" | "isinstance" | "issubclass" | "hasattr" | "getattr" | "setattr"
        | "delattr" | "super" | "property" | "staticmethod" | "classmethod" | "enumerate" | "zip"
        | "map" | "filter" | "sorted" | "reversed" | "min" | "max" | "sum" | "abs" | "round"
        | "open" | "input" | "repr" | "id" | "hash" | "callable" | "iter" | "next" | "vars" | "dir"
        | "Exception" | "ValueError" | "TypeError" | "KeyError" | "IndexError" | "AttributeError"
        | "RuntimeError" | "StopIteration" | "NotImplementedError" | "OSError" | "IOError"
        | "True" | "False" | "None")
}
